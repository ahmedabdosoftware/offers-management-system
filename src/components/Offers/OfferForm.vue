<template>
  <v-dialog v-model="dialog" max-width="700px">
    <v-card class="custom-card">
      ...
      <v-card-text>
        <VeeForm :validation-schema="schema" v-slot="{ errors }" @submit="onSubmit">
          <v-container>
            <v-row>
              <v-col cols="12" md="6">
              ...
              <!-- 🔹 فايل خارج VeeValidate -->
              <v-col cols="12" md="6">
                <v-file-input
                  label="Upload Offer File"
                  outlined
                  accept=".pdf,.doc,.docx"
                  @change="onOfferFileChange"
                  :error="!!offerFileError"
                  :error-messages="offerFileError"
                />
              </v-col>
              ...
              <v-col cols="12" md="6">
                <v-file-input
                  ...
                  :error-messages="clientLogoError"
                />
              </v-col>
            </v-row>
          </v-container>
          ...
          <v-card-actions>
            ...
            <v-btn color="grey" class="action-btn" @click="dialog = false">Cancel</v-btn>
          </v-card-actions>
        </VeeForm>
      </v-card-text>
    </v-card>
...
export default {
  ...
  methods: {
    // offer File :- Check Size
    onOfferFileChange(file) {
      if (!file) {
        this.offerFileError = "Offer file is required";
        this.offerFile = null;
        return;
      }
      if (file.size > 250 * 1024) {
        this.offerFileError = "Offer file must be less than 250KB";
        this.offerFile = null;
        return;
      }
      this.offerFileError = "";
    ...
    // Client logo :- Check Size
    onClientLogoChange(file) {
      if (!file) {
        this.clientLogoError = "Client logo is required";
        this.clientLogo = null;
        return;
      }
      if (file.size > 250 * 1024) {
        this.clientLogoError = "Client logo must be less than 250KB";
        this.clientLogo = null;
        return;
      }
      this.clientLogoError = "";
      this.clientLogo = file;
    },

    // send Form Data
    onSubmit(values) {
      if (!this.offerFile || !this.clientLogo) {
        if (!this.offerFile) this.offerFileError = "Offer file is required";
        if (!this.clientLogo) this.clientLogoError = "Client logo is required";
        return;
      }
